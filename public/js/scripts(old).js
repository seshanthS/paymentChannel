...
function signTransaction(){
    var amount = $("#amountField").val//.toString();
    ...
    try{
        ...
        accounts = window.web3.eth.getAccounts().then(account=>{
        	console.log(account)
        	var txData = window.web3.eth.abi.encodeFunctionCall(functionAbi,[shopId,amount,account[0]]);
        	console.log(txData);
        	var tx = {
            from: account[0],
            to: contractAddress,
         ...
         console.log(tx)
         window.web3.eth.signTransaction(tx, account[0]).then(console.log);
